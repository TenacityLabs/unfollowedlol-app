...
export default function UserAvatar({ src, fallback = '/default.jpg', alt, width, height, className }: { src: string, fallback?: string, alt: string, width: number, height: number, className: string }) {
  ...
  return (
    <Image
      ...
      height={height}
      className={className}
      onError={() => setImageSrc(fallback)} // Change to fallback if there's an error
    />
  );
}